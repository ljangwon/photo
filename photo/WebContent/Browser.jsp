...
<%!
	...
	*/
	...
	public static String toHangul(String _str)
	{
		...
		try
		{
			strRet = java.net.URLEncoder.encode(_str, "euc-kr"); // , "euc-kr"
	...
	public static String fromHangul(String _str)
	{
		...
		try
		{
			strRet = java.net.URLDecoder.decode(_str, "euc-kr"); // , "euc-kr"
			strRet = new String(strRet.getBytes("8859_1"),"KSC5601");
		}
...
<% 		}
		...
		// Download file
			...
			if (!isAllowed(f, false)){
				...
				response.setHeader("Content-Disposition", "attachment;filename=\"" + f2.getName()
						+ "\"");
...
//			System.out.println("*** fname-2 : " + toHangul3(fname));
//			System.out.println("*** fname-3 : " + fromHangul3(fname));
//			System.out.println("*** fname-4 : " + toHangul(fname));
//			System.out.println("*** fname-5 : " + fromHangul(fname));
...
<body>
<b>Upload of[1] <%=fname%></b><br><br>
Upload aborted.</body>
</html><%
			}
			else if (info != null && (info.totalSize != info.currSize || info.currSize == 0)) {
				%>
<META HTTP-EQUIV="Refresh" CONTENT="<%=UPLOAD_MONITOR_REFRESH%>;URL=<%=browser_name %>?uplMonitor=<%=toHangul(fname)%>">
</head>
<body>
<b>Upload of[2] <%=fname%></b><br><br>
<center>
...
<%
		...
		if (dir_view && request.getAttribute("dir") != null) {
			File f = new File("" + fromHangul((String)request.getAttribute("dir")));
...
<%
			...
			if (f.getParent() != null) {
				...
				out.println(" &nbsp;<a href=\"" + browser_name + "?sort=" + sortMode + "&amp;dir="
						+ toHangul(f.getParent()) + "\">" + FOL_IMG + "[..]</a>");
				out.print("</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td></td></tr>");
			}
	...
	<p align=center>
		...
		<%=convertFileSize(totalSize)%></b><b> in <%=fileCount%> files in <%= dir2linkdir(fromHangul((String) request.getAttribute("dir")), browser_name, sortMode)%>
		</b>
	</p>
		<%
